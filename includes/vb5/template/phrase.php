...
class vB5_Template_Phrase
{
	...
	public function replacePlaceholders(&$content)
	{
		...
		while (!is_null($placeholder_id = key($this->stack)))
		{
			...
			{
				//don't want to pull this out of the loop because we may not even have a notice phrase
				if(!isset($replace_map))
				{
					$replace_map = array (
				...
				$phraseInfo[0] = str_replace(array_keys($replace_map), $replace_map, $phraseInfo[0]);
			}
