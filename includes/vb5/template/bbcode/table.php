...
class vB5_Template_BbCode_Table
{
	...
	*	* addunit -- unit to add to the value entered -- not applied if the value is a percentage (ends with %).  Only applies
	*			to css attributes (css field must also be set).
	*	*	wysiwygattr -- if true this will be rendered as an attribute for the wysiwyg render.  The wysiwyg table html sets
	*			table attributes instead of styles, but that's not how html5 does business.  The editor appears to work so I
	*			don't want to change it, but we should render our display per standards.  Only affects rendering for the wysiwyg parser
	...
	*/
	public function parseTableTag($content, $params = '')
	{
		...
		$output = $this->parseTableRows($content, $this->getClassList($table_params), $table_params);
		if ($output === '')
		{
			return '';
		}
		...
		// remove table params that should not be applied directly to the table
		// element. They were already passed to parseTableRows and parseTableCells,
		// and applied directly to each cell.
		// keep this syncronized with parseTableCells
		foreach (array('border-width', 'padding') AS $item)
		{
			if (isset($table_params['css'][$item]))
			{
				unset($table_params['css'][$item]);
			}
		}

		// we need to add margin auto so the center align for the table works
		if (isset($table_params['attributes']['align']) AND $table_params['attributes']['align'] == 'center')
		{
			$table_params['css']['margin-left'] = 'auto';
			$table_params['css']['margin-right'] = 'auto';
		}
	...
	*/
	protected function modifyTableParams(array $table_params)
	{
		...
		{
			if(isset($def['addunit']) AND isset($def['css']))
			{
				$field = $def['css'];
				//add units to height and width style params if they aren't percentages.
				if (isset($table_params['css'][$field]) AND substr($table_params['css'][$field], -1) != '%')
				{
					$table_params['css'][$field] .= 'px';
				}
			}
		}
	...
	* @param	array	Array of table params (from modifyTableParams)
	*
	* @return	string	HTML output of the rows
	*/
	protected function parseTableRows($content, array $table_classes, array $table_params)
	{
		...
		foreach ($rows AS $row)
		{
			...
			$cell_output = $this->parseTableCells($row['content'], $cell_class_pushdown, $table_params);
	...
	*/
	protected function parseTableCells($content, array $row_classes, array $table_params)
	{
		...
		foreach ($cells AS $cell)
		{
			...
			// add certain table params to cell params so they actually
			// affect how the table is displayed
			// keep this syncronized with parseTableTag
			foreach (array('border-width', 'padding') AS $item)
			{
				if (isset($table_params['css'][$item]))
				{
					$cell_params['css'][$item] = $table_params['css'][$item];
				}
			}
