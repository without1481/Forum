...
class vB5_Frontend_Controller_CreateContent extends vB5_Frontend_Controller
{
	...
	{
		$input = array(
			'eventstartdate' => ((isset($_POST['eventstartdate']) AND !empty($_POST['eventstartdate'])) ? $this->userTimeStrToUnixtimestamp($_POST['eventstartdate']) : 0),
			'eventenddate'   => ((isset($_POST['eventenddate']) AND !empty($_POST['eventenddate'])) ? $this->userTimeStrToUnixtimestamp($_POST['eventenddate']) : 0),
			'allday'         => (isset($_POST['is_all_day']) ? (bool)$_POST['is_all_day'] : false), // if true, this cancels 'eventenddate' above
	...
	protected function actionTextNodeInternal($apiClass)
	{
		...
		else
		{
			...
			if(!$this->createNewNode($apiClass, $textData, $publish, $input))
			{
				return;
			}
		}
	 ...
	 */
	private function createNewNode($apilib, $data, $publish, $input, $alert=null)
	{
	...
	private function userTimeStrToUnixtimestamp($strTime)
	{
		/*
		...
		$api = Api_InterfaceAbstract::instance();
		$apiResult = $api->callApi('user', 'userTimeStrToUnixtimestamp', array($strTime));
	...
	public function actionPoll()
	{
		...
		$input = array(
			...
			'nodeid'          => (isset($_POST['nodeid']) ? trim(intval($_POST['nodeid'])) : 0),
			'ret'             => (isset($_POST['ret']) ? trim(strval($_POST['ret'])) : ''),
			'timeout'         => ((isset($_POST['timeout']) AND !empty($_POST['timeout'])) ? $this->userTimeStrToUnixtimestamp($_POST['timeout']) : 0),
	...
	public function actionGallery()
	{
		...
		if ($input['nodeid'])
		{
			...
			$galleryData = array(
				...
				'viewperms' 			=> (isset($_POST['viewperms']) ? (int)$_POST['viewperms'] : 2),
				'prefixid'        => $input['prefixid'],
				'iconid'          => $input['iconid'],

				//not used for updateFromWeb,  probably needed for updates
				'parseurl'        => $input['parseurl'],
		...
		else
		{
			...
			if(!$this->createNewNode($apiClass, $galleryData, $publish, $input, $alert))
			{
				return;
			}
		}
