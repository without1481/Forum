...
define('CVS_REVISION', '$RCSfile$ - $Revision: 94108 $');
define('NOZIP', 1);
...
if ($_REQUEST['do'] == 'liftban')
{
	...
	print_form_header('modcp/banning', 'doliftban');
...
if ($_POST['do'] == 'dobanuser')
{
	...
	$userdm->set('displaygroupid', 0);
	$userdm->set('status', ''); // clear status, VBV-15853
...
if ($_REQUEST['do'] == 'banuser')
{
	...
	print_form_header('modcp/banning', 'dobanuser');
...
if ($_REQUEST['do'] == 'editreason')
{
	...
	print_form_header('modcp/banning', 'updatereason');
	print_table_header($vbphrase['ban_user']);
...
if ($_REQUEST['do'] == 'modify')
{
	...
	function construct_banned_user_row($user, $canunbanuser)
	{
		...
		$userpage = (can_administer('canadminusers') ? 'admincp/user.php' : 'modcp/user.php');

		$cell = array();
		$cell[] = "<a href=\"$userpage?do=edit&amp;u=$user[userid]\"><b>$user[username]</b></a>";
		if ($user['bandate'])
		{
			$cell[] = $user['adminid'] ? "<a href=\"$userpage?do=edit&amp;u=$user[adminid]\">$user[adminname]</a>" : $vbphrase['n_a'];
		...
		if ($canunbanuser)
		{
			$cell[] = construct_link_code(
				$vbphrase['lift_ban'],
				"modcp/banning.php?do=liftban&amp;u=$user[userid]",
				false,
				'',
				false,
				false
		...
		$cell[] = construct_link_code(
			!empty($user['reason']) ? $user['reason'] : $vbphrase['n_a'],
			"modcp/banning.php?do=editreason&amp;userid=" . $user['userid'],
			false,
			'',
			false,
			false
	...
	if ($vbulletin->db->num_rows($tempusers))
	{
		...
		{
			print_form_header('modcp/banning', 'banuser');
	...
	if ($permusercount['count'])
	{
		...
		if ($pagecount > 1)
		{
			...
			for ($thispage = 1; $thispage <= $pagecount; $thispage++)
			{
				...
				{
					$pagenav .= " <a href=\"modcp/banning.php?do=modify&amp;page=$thispage\" class=\"normal\">$thispage</a> ";
				}
			}
	...
	if (!$havebanned)
	{
		if ($canbanuser)
		{
			print_stop_message('no_users_banned_from_x_board_click_here', '<b>' . $vbulletin->options['bbtitle'] . '</b>', 'modcp/banning.php?do=banuser');
		}
		else
		{
