 ...
 *
 * @package	vBulletin
 * @version	$Revision: 94270 $
 * @date		$Date: 2017-06-15 18:22:23 -0700 (Thu, 15 Jun 2017) $
 */
class vB_Datastore
{
	...
	*/
	...
	public function buildUserGroupCache($usergroupinfo)
	{
		...
		// upgrade classes may pass in a usergroup db fetch result array rather
		// than an array keyed by usergroup that permissioncontext expects. VBV-16058
		$keyedArray = array();
		foreach ($usergroupinfo AS $data)
		{
			if (isset($data['usergroupid']))
			{
				$keyedArray[$data['usergroupid']] = $data;
			}
		}

		// set info needed
		$this->build('usergroupcache', serialize($keyedArray), 1);
		return $keyedArray;
	}
}
