...
class vB_Tags
{
	...
	*/
	...
	{
		$usercontext = vB::getUserContext();
		return $usercontext->getChannelPermission('forumpermissions', 'cantagown', $this->nodeid);
	}
	...
	*/
	public function addTagsToContent($taglist)
	{
		...
		if (!empty($inserts))
		{
			...
			$this->assertor->assertQuery('insertignoreValues', array('table' => 'tag', 'values' => $taglistInsert));
		}
	...
	*/
	protected function loadContentInfo()
	{
		...
		$query = $this->assertor->assertQuery('vBForum:getTags', array('nodeid' => $this->nodeid));
