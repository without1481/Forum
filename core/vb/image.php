...
*
* @package 		vBulletin
* @version		$Revision: 94529 $
* @date 		$Date: 2017-07-13 10:51:35 -0700 (Thu, 13 Jul 2017) $
*
*/
abstract class vB_Image
{
	...
	*/
	...
	public function fetchImageExtension($file)
	{
		...
		if (function_exists('curl_getinfo'))
		{
			...
			curl_setopt($connection, CURLOPT_CAINFO, DIR . '/includes/paymentapi/ca-bundle.crt');
			...
			if (!empty($type))
			{
				...
				{
					$type = $this->extension_map[$type[1]];
					return $this->type_to_canonical_extension[$type];
				}
			}
		}
	...
	*/
	...
	public function fetchImageExtensions()
	{
		// We can support these as images at fullsize:
	...
	public function getExtensionFromFileheaders($location)
	{
	 ...
	 * didn't create.
	 */
	public function loadImage($location)
	{
		/*
			Note
	 ...
	 * Currently used by upgrade scripts to check if GhostScript is installed
	 * and configured in the delegates file when using ImageMagick library.
	 * @return	bool
	 */
