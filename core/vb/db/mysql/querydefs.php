...
class vB_dB_MYSQL_QueryDefs extends vB_dB_QueryDefs
{
	...
	* * */
	protected $query_data = array(
		...
		'template_fetch_css_templates' => array(
			vB_dB_Query::QUERYTYPE_KEY => vB_dB_Query::QUERY_SELECT,
			'query_string' => "
	...
	public function fetchUserinfo($params, $db, $check_only = false)
	{
		...
		else
		{
			...
			if (in_array(vB_Api_User::USERINFO_PROFILEPIC, $params['option']))
			{
				$select[] = 'customprofilepic.userid AS profilepic';
				$select[] = 'customprofilepic.dateline AS profilepicdateline';
				$select[] = 'customprofilepic.width AS ppwidth';
				$select[] = 'customprofilepic.height AS ppheight';

				$join[] = 'LEFT JOIN ' . TABLE_PREFIX . 'customprofilepic AS customprofilepic ON (user.userid = customprofilepic.userid)';
			}
