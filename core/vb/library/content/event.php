...
class vB_Library_Content_Event extends vB_Library_Content_Text
{
	...
	{
		$existing = $this->nodeLibrary->getNode($nodeid);
	...
	public function getEndOfDayUnixtime($timestamp, $userid = false)
	{
		$userApi = vB_Api::instanceInternal('user');
		$dateString = $userApi->unixtimestampToUserDateString($timestamp, $userid, "Y-m-d");
		$endOfDayString = $dateString['datestr'] . " 11:59:59 PM";
		$endOfDayTimestamp = $userApi->userTimeStrToUnixtimestamp($endOfDayString, $userid);
	...
	public function checkEventData($data, $existing = false)
	{
		...
		if (isset($data['allday']))
		{
			if ($data['allday'])
			{
				// This will be set to the "end of day" via code below.
				$data['eventenddate'] = 0;
			}
		}
		...
		if (!$existing OR isset($data['eventstartdate']) OR isset($data['eventenddate']))
		{
			...
			if (empty($data['eventenddate']) OR !empty($data['allday']))
			{
				// force set it in case it's missing so that on update() to define behavior on update()
				//$data['eventenddate'] = 0;
				// Preserve old behavior - having no/empty eventenddate is shorthand for "all day" events.
				$enddate = $this->getEndOfDayUnixtime($data['eventstartdate']);
				// If, for some weird reason, they began the event at 11:59:59PM as an "all day" event,
				// just add 1 second & tick over to the next day to ensure that eventenddate is always
				// later than startdate.
				if ($enddate <= $data['eventstartdate'])
				{
					$enddate = $data['eventstartdate'] + 1;
				}
			...
			else
			{
				...
				// eventenddate may not be X later than eventstartdate.
