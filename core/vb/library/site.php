...
class vB_Library_Site extends vB_Library
{
	...
	protected function prepareNavbarData(array &$data, $url = false, $edit = false, $channelId = 0)
	{
		...
		foreach ($data AS $k => &$item)
		{
			if (is_array($item) AND isset($item['url']))
			{
				...
				if (!$found_current AND !empty($url))
				{
					...
					if(
						strtolower($url) == strtolower($itemUrl) OR
						(strlen($url) > strlen($itemUrl) AND strtolower(substr($url, 0, -(strlen($url) - strlen($itemUrl)))) == strtolower($itemUrl)))
					{
		...
		if (!$found_current)
		{
			...
			//maps the "channel type" to the tab that should be displayed for that channel
			$channelTabMap = array(
				'blog' => 'blogs',
				'group' => 'social-groups',
				'article' => 'articles',
			...
			//the urls to apply the channel mapping to (if we have a channel)
			$specialChannelCases = array(
				'/new-content',
				'/node',
			);

			//other special case urls mapping from the url to the tab
			$specialCases = array(
				'/blogadmin' => 'blogs',
				'/sgadmin' => 'social-groups',
			);

			$channelId = (int) $channelId;
			if ($channelId > 0)
			{
				foreach ($specialChannelCases AS $urlsegment)
				{
					if (strpos($url, $baseurl_short . $urlsegment) === 0)
					{
						$type = $this->getChannelType($channelId);
						if (isset($channelTabMap[$type]))
						{
							$setCurrentTab = $channelTabMap[$type];
						}
					}
				}
			}
			...
			if (!$setCurrentTab)
			{
				foreach ($specialCases AS $urlsegment => $tab)
				{
					if (strpos($url, $baseurl_short . $urlsegment) === 0)
					{
						$setCurrentTab = $tab;
			...
			// it's not clear that we ever have a channel we don't want to
			// set the tab, but it's possible that there are cases...
			// special case: social groups, categories & topics
			// social group routes do not maintain the 'social-groups' bit in the URL
			// (this no longer appears to be the case, which means that this may no longer be
			// necesary).
			if (!$setCurrentTab AND $channelId > 0)
			{
				$type = $this->getChannelType($channelId);
				if($type == 'group' AND isset($channelTabMap[$type]))
				{
					$setCurrentTab = $channelTabMap[$type];
				}
			}
