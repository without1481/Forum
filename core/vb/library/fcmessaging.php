...
class vB_Library_FCMessaging extends vB_Library
{
	...
	public function queueMessage($recipientIds, $messageType, $extra = array())
	{
		...
		switch($messageType)
		{
			case self::MESSAGE_TYPE_PRIVATEMESSAGE:
				 ...
				 */
				$title = vB_String::unHtmlSpecialChars($extra['title']);
				// get rid of bbcode. In particular, stuff like [font][color] etc.
				$body = vB_String::stripBbcode($extra['rawtext']);
				$body = vB_String::getPreviewText($body, $limit);
				// getPreviewText() puts it through htmlspecialchars(), so undo it. See note above.
				$body = vB_String::unHtmlSpecialChars($body);
				$body = vB_String::unHtmlSpecialChars($extra['username']) . ": " . $body;
				// Convert to UTF-8 for JSON, and truncate again to $limit in case conversion made it longer.
				if (!$currentlyUTF8)
				{
			...
			case self::MESSAGE_TYPE_NOTIFICATION:
				...
				// Todo: we may want to offload this to the vB_Notification_X class as well,
				...
				$title = vB_String::unHtmlSpecialChars($extra['title']);
				$body = vB_String::unHtmlSpecialChars($extra['body']);
	...
	private function postToFCMServer($postData, $skipResultProcessing = false)
	{
		...
		$vurl->set_option(VURL_TIMEOUT, 5); // Timeout of 5s

		// If google redirects us for some reason, I'm not sure if we want to just follow it
	...
	private function registerMessageQueue($postData, $sendAfter = NULL)
	{
		$registration_ids = array();
		...
		if (is_null($sendAfter))
		{
			// No specific time means send it ASAP.
			$sendAfter = vB::getRequest()->getTimeNow() - 1;
		}
		...
		if (empty($messageid))
		{
			...
			$this->logError(
				"Failed to register FCMessage to queue because we could not generate the queue messageid",
				array(
					"message_data" => $jsonPostData,
					"message_hash" => $message_hash,
				)
		...
		$query = $this->assertor->assertQuery(
			'vBForum:apiclient_devicetoken',
			array("devicetoken" => $registration_ids)
		);
		foreach ($query AS $__row)
		{
			$this->assertor->insertIgnore(
				...
				array(
					'recipient_apiclientid' => $__row['apiclientid'],
					'messageid'  => $messageid,
					'sendafter'  => $sendAfter,
					'status'     => "ready",
				)
