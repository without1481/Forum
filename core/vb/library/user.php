...
class vB_Library_User extends vB_Library
{
	...
	public function sendActivateEmail($userid)
	{
		...
		$maildata = vB_Api::instanceInternal('phrase')->fetchEmailPhrases(
			'activateaccount',
			array($username, $vboptions['bbtitle'], $vboptions['frontendurl'], $userinfo['userid'], $activateid, $vboptions['webmasteremail']),
			array($username), $userinfo['languageid']
		);
