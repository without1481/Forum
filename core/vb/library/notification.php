...
class vB_Library_Notification extends vB_Library
{
	...
	{
		if (empty($this->fcmQueue))
		{
			return;
		}
		/*
									$this->fcmQueue[$key] = $notificationData;
		 */
		/*
		...
		$recipients = array(); // array of userids grouped by notification typename, then languageid
		$fcmPayloadHelper = array(); // grouped by typename
			...
			if ($__class::FCM_SUPPORTED)
			{
				if (empty($fcmPayloadHelper[$__typename]))
				{
					/*
						...
						payload.
					 */
					$fcmPayloadHelper[$__typename] = array(
				...
				{
					$recipients[$__typename] = array();
				}
				if (empty($recipients[$__typename][$__languageid]))
				{
					$recipients[$__typename][$__languageid] = array();
				}
	...
	protected function sendLegacyEmailNotification($data)
	{
		...
		if (!empty($contentnodeid) AND empty($recipientIndependentDataArray[$contentnodeid]))
		{
			...
			if (!empty($currentNode['userinfo']['username']))
			{
				$currentNodeUsername = $currentNode['userinfo']['username'];
			}
			else if(!empty($currentNode['authorname']))
			{
				$currentNodeUsername = $currentNode['authorname'];
			}
			else
			{
				// I haven't hit this case while testing, but let's have a fallback to "Guest"...
				$currentNodeUsername = "Guest";
			}
			...
			$recipientIndependentDataArray[$contentnodeid] = array(
				'authorname' => $currentNodeUsername,
