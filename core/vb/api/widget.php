		...
		foreach($containerConfig['tab_data'] AS $__tabnum => $__tab)
		{
			...
			foreach ($__tab['widgets'] AS $__widget)
			{
				...
				if (!empty($__widget['widgetinstanceid'])) // this one has been configured via SB
				{
					...
					if (!empty($tabbedContainerSubModules[$__wiid]['template']))
					{
		...
		if ($configFields)
		{
			foreach ($configFields AS $configField)
			{
				...
				switch ($configField['validationtype'])
				{
					...
					case 'method':
						// special handling to convert the weird inputs
						if ($configField['name'] == "module_viewpermissions")
						{
							$cleaned = $this->clean_config_module_viewpermissions($data["module_viewpermissions"]);
						}
						else
						{
							throw new vB_Exception_Api('Not implemented');
...
//			'widgetinstanceid' => $widgetinstanceid,
	...
	public function saveSearchWidgetConfig($data)
	{
		...
		$yesNoFields = array('hide_avatars', 'hide_module_when_empty', 'displayFooter');
