...
//	const FILTER_VIEW_CHANNEL = 'channel';
	...
	public function getChannels(
		$flat = false,
		$queryOptions = array('exclude_categories' => false, 'include_protected' => false),
		$skipcache = false,
		$topLevelChannel = 0
	)
	{
	...
	private function json2criteria(&$json)
	{
		...
		if (!empty($search_structure['eventenddate']) AND is_array($search_structure['eventenddate']))
		{
			...
			if ( !empty($search_structure['eventenddate']['from']) )
			{
				...
				if(!empty($__dateline))
				{
					$criteria->add_filter('eventenddate', vB_Search_Core::OP_GT, $__dateline);
			...
			if ( !empty($search_structure['eventenddate']['to']) )
			{
				...
				if(!empty($__dateline))
				{
					$criteria->add_filter('eventenddate', vB_Search_Core::OP_LT, $__dateline);
		...
		if (!empty($search_structure['eventstartdate']))
		{
			...
			if (is_array($search_structure['eventstartdate']))
			{
				if ( !empty($search_structure['eventstartdate']['from']) )
				{
					...
					if(!empty($__dateline))
					{
						$criteria->add_filter('eventstartdate', vB_Search_Core::OP_GT, $__dateline);
				...
				if ( !empty($search_structure['eventstartdate']['to']) )
				{
					...
					if(!empty($__dateline))
					{
						$criteria->add_filter('eventstartdate', vB_Search_Core::OP_LT, $__dateline);
