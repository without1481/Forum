...
class vB_Api_User extends vB_Api
{
		...
		if (empty($userInfo))
		{
			$userInfo = vB_User::fetchUserinfo($userid,
				array(
					...
					vB_Api_User::USERINFO_PROFILEPIC,
	...
	protected function getCustomFieldValue($customField, $fieldValues)
	{
		switch ($customField['type'])
		{
			...
			case 'checkbox':
				$selected = intval($fieldValues['field' . $customField['profilefieldid']]);
	...
	public function fetchUserSettings($userid = false)
	{
		...
		if ($userid == $currentUserId OR !$this->hasAdminPermission('canadminusers'))
		{
			...
			if(!empty($config['Security']['mfa_enabled']))
			{
				...
				//MFA is irrelevant.
			 	if($thisUserContext->isAdministrator() OR $thisUserContext->isModerator())
				{
					//this is true if
	...
	*/
	...
	public function memberList($criteria = array())
	{
		...
		{
			$pagingInfo['records'] = vB::getDbAssertor()->getField('user', array(
...
//					}
	 ...
	 *		adminoverride if the user is an admin.
	 * @return integer New or updated userid.
	 */
	...
	public function saveSignature($signature, $filedataids = array())
	{
		...
		{
			vB::getDbAssertor()->assertQuery('incrementFiledataRefcountAndMakePublic', array('filedataid' => $img2Filedataids));
		}
		$oldImg2Filedataids = $this->getFiledataIdsFromSignature($userinfo['signature']);
		if (!empty($oldImg2Filedataids))
		{
			vB::getDbAssertor()->assertQuery('decrementFiledataRefcount', array('filedataid' => $oldImg2Filedataids));
		}
 ...
 */
	...
	private function loginInternal($userInfo, $username, $passwords, $extraAuthInfo, $logintype)
	{
		...
		$auth = $this->verifyAuthentication($userInfo, $passwordInfo);
		if (!$auth)
		{
			$this->processLoginError($username, $logintype, $strikes);
		}
	 ...
	 */
	private function verifyAuthentication($userInfo, $passwords)
	{
	 ...
	 *
	 * @param	array $ignorelist -- ignored users ids.
	 *
	 ...
	 * @return integer
	 */
	public function fetchTimeOffset($adjustForServer = false, $userid = false)
	{
		if ($userid === false)
		{
			$userInfo = vB::getCurrentSession()->fetch_userinfo();
	 ...
	 */
	public function userTimeStrToUnixtimestamp($strTime, $userid = false)
	{
		/*
		...
		$offset = $this->fetchTimeOffset(false, $userid);
	 ...
	 */
	public function unixtimestampToUserDateString($timestamp, $userid = false, $format = "Y-m-d H:i:s")
	{
		/*
	...
	{
		$fields = array('token', 'scheme', 'secret', 'securitytoken_raw', 'fbaccesstoken');
	...
	{
		$fields = array(
			'coppauser', 'parentemail', 'passworddate', 'logintype', 'ipaddress', 'passworddate', 'email', 'referrerid',
