	...
	private function getSubContentsArrayFromSearchResult($result)
	{
		...
		foreach ($result['results'] AS $__article)
		{
			...
			if ($isChannel)
			{
				...
				{
					$channelURLsByRouteid[$__article['routeid']] = vB5_Route::buildUrl($__article['routeid'] . "|fullurl", $__article);
				}
			...
			else
			{
				...
				{
					$channelURLsByRouteid[$parentRouteid] = vB5_Route::buildUrl($parentRouteid . "|fullurl", $parentNode);
				}
			...
			$articleUrl = vB5_Route::buildUrl($__article['routeid'] . "|fullurl", $routeData);
...
20 rows in set (0.00 sec)
	...
	public function content_view($nodeid, $do = "", $contentpagenum = 1, $page = 1)
	{
		...
		if ($isChannel)
		{
			...
			$channelUrl = vB5_Route::buildUrl($node['routeid'] . "|fullurl", $routeData);
		...
		else
		{
			...
			$parentUrl = vB5_Route::buildUrl(
				$parentCategoryNode['routeid'] . "|fullurl", $parentCategoryNode
			);
			...
			$articleUrl = vB5_Route::buildUrl($node['routeid'] . "|fullurl", $routeData);
	...
	public function sectionlist()
	{
		...
		foreach($channels AS $__channel)
		{
			...
			$channelUrl = vB5_Route::buildUrl($__channel['routeid'] . "|fullurl", $__channel);
			$return[] = array(
