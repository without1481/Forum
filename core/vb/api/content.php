 ...
 * @version $Id: content.php 94814 2017-08-10 00:32:42Z ksours $
 * @access public
 */
abstract class vB_Api_Content extends vB_Api
{
	...
	public function cleanInput($data, $nodeid = false)
	{
		...
		else if (!empty($checkNodeid))
		{
			...
			$limits = $userContext->getChannelLimits($checkNode['channelid']);
	...
	public function delete($nodeid)
	{
		...
		if (!$this->library->validate($data, vB_Library_Content::ACTION_DELETE, $nodeid))
		{
	...
	public function update($nodeid, $data)
	{
		...
		//this was previously done in the library function -- but we need
		//to be able to set approved/unapproved in the backend.  We don't
		//want to set it through the public API updates
		unset($data['approved']);
		unset($data['showapproved']);
