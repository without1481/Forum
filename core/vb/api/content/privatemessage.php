 ...
 * @version $Id: privatemessage.php 95123 2017-08-28 23:01:48Z ksours $
 * @access public
 */
	...
	public function addPMRecipients($pmid, $recipients)
	{
		...
		*/
		...
		foreach ($recipientsCopy AS $_key => $_userid)
		{
			...
			$result = $this->library->checkCanReceivePM(array('userid' => $_userid));
		...
		foreach ($emailData AS $_data)
		{
			...
			$maildata = $phraseApi->fetchEmailPhrases(
				'privatemessage',
				array($_data['username'], $senderUsername,	$url, $lastNodePreviewText, $bbtitle),
				array($bbtitle)
			);
