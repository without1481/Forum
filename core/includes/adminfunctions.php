...
function print_cp_header($title = '', $onload = '', $headinsert = '', $marginwidth = 0, $bodyattributes = '', $base = '', $titlenote= '')
{
	...
	<link rel=\"stylesheet\" type=\"text/css\" href=\"core/cpstyles/" . $options['cpstylefolder'] . "/controlpanel.css?v={$options['simpleversion']}\" />" . iif($headinsert != '', "$headinsert") . "
	<style type=\"text/css\">
		...
		.b-pagination a {
			...
			color: " .  vB_Template_Runtime::fetchStyleVar('button_secondary_text_color.color') . ";
			*/ "
		}
	...
	var IMGDIR_MISC = \"core/cpstyles/" . $vbulletin->options['cpstylefolder'] . "\";
	...
	<script type=\"text/javascript\" src=\"core/clientscript/vbulletin_ajax_suggest.js\"></script>
...
function print_checkbox_row($title, $name, $checked = true, $value = 1, $labeltext = '', $onclick = '')
{
	...
	print_label_row(
		...
		"<div id=\"ctrl_$name\"><label for=\"{$name}_$uniqueid\" class=\"smallfont\"><input type=\"checkbox\" name=\"$name\" id=\"{$name}_$uniqueid\" value=\"$value\" tabindex=\"1\"" . iif($onclick, " onclick=\"$onclick\"") . iif($vb5_config['Misc']['debug'], " title=\"name=&quot;$name&quot;\"") . iif($checked, ' checked="checked"') . " />$labeltext</label></div>",
		'', 'top', $name
	);
}
...
function construct_help_button($option = '', $action = NULL, $script = '', $helptype = 0, $helpOptions = array())
{
	...
	$linkbody = $linkphrase . '<img src="core/cpstyles/' . $vboptions['cpstylefolder'] . '/cp_help.' . $vboptions['cpstyleimageext'] .
...
{
	return print_cp_redirect(get_redirect_url($file, $extra, $route), $timeout);
}

function print_cp_redirect_with_session($file, $extra = array(), $route = 'admincp')
{
	$args = array();
	parse_str(vB::getCurrentSession()->get('sessionurl'),$args);
	$extra = array_merge($args, $extra);
	return print_cp_redirect(get_redirect_url($file, $extra, $route), 1);
}
