...
*
* @package 		vBulletin
* @version		$Revision: 91234 $
* @date 		$Date: 2016-10-25 18:27:08 -0700 (Tue, 25 Oct 2016) $
*
*/
	...
	function parse_whitespace_newlines($text, $do_nl2br = true)
	{
		...
		if ($this->is_wysiwyg('ie'))
		{
			...
			$text = preg_replace_callback($rematch_find,
				array($this, 'bbcodeRematchTagsCallback1'), $text);
			...
			$text = preg_replace_callback($rematch_find,
				array($this, 'bbcodeRematchTagsCallback2'), $text);

			$text = '<p>' . preg_replace('#(\r\n|\n|\r)#', "</p>\n<p>", ltrim($text)) . '</p>';

			if (strpos('[/list', strtolower($text))) // workaround bug #22749
			{
				$text = preg_replace('#(\[list(=(&quot;|"|\'|)(.*)\\3)?\])(((?>[^\[]*?|(?R))|(?>.))*)(\[/list(=\\3\\4\\3)?\])#siU',
					array($this, 'removeWysiwygBreaksPregMatch'), $text
				);
			}
	...
	*/
	function parse_bbcode($input_text, $do_smilies, $do_imgcode, $do_html = false)
	{
		...
		$text = preg_replace_callback('#\[(code|php|html)\](.*)\[/\\1\]#siU',
			array($this, 'stripSmiliesCallback'), $text);
