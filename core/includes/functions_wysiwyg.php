...
function convert_wysiwyg_html_to_bbcode($text, $allowhtml = false, $p_two_linebreak = false)
{
	...
	$text = preg_replace_callback('#(<[^<>]+ (src|href))=(\'|"|)??(.*)(\\3)#siU',// make < and > safe in inside URL/IMG tags so they don't get stripped by strip_tags
		function($matches)
		{
	...
	$text = preg_replace_callback('#<img[^>]+smilieid="(\d+)".*>#siU',                 // smilies
		function($matches)
		{
	...
	$text = preg_replace_callback('#<img[^>]+src=(\'|")(.*)(\\1).*>#siU',              // img tag
		function($matches)
		{
