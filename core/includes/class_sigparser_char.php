...
class vB_SignatureParser_CharCount extends vB_SignatureParser
{
	...
	/**
	 * @var int Computed ratio for calculating size. Set by handle_bbcode_size and used by handleBbcodeSizeCallback.
	 */
	protected $computedRatio = 0;
	...
	*/
	...
	{
		parent::__construct($registry, $tag_list, $userid, $append_custom_tags);
	...
	*/
	...
	function handle_bbcode_size($value, $size)
	{
		...
		$this->computedRatio = $computed_ratio;

		return preg_replace_callback('#([^\n]+)#', array($this, 'handleBbcodeSizeCallback'), $value);
	}

	/**
	 * Callback for preg_replace_callback in handle_bbcode_size
	 */
	function handleBbcodeSizeCallback($matches)
	{
		return $this->handle_bbcode_size_callback($matches[1], $this->computedRatio);
	}
	...
	*/
	function handle_bbcode_img($bbcode, $do_imgcode, $has_img_code = false, $fulltext = '')
	{
		...
		if ($has_img_code & BBCODE_HAS_IMG)
		{
			...
			{
				$bbcode = preg_replace_callback('#\[img\]\s*(https?://([^*\r\n]+|[a-z0-9/\\._\- !]+))\[/img\]#iU', array($this, 'handleStandardTagCallback'), $bbcode);
			}
		}
	 ...
	 */
	protected function handleStandardTagCallback($matches)
	{
		return $this->handle_standard_tag($matches[1], '');
	}
}
