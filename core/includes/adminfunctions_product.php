...
function delete_product($productid, $compliant_35 = false, $for_product_upgrade = false)
{
	...
	if (!$for_product_upgrade)
	{
		...
		if ($stylevarids)
		{
			$assertor->delete('stylevar', array('stylevarid' => $stylevarids, 'styleid' => -1));
			$assertor->delete('vBForum:stylevardfn', array('stylevarid' => $stylevarids));
		}
...
function get_product_export_xml($productid)
{
	...
	foreach ($settinggroup AS $grouptitle => $group)
	{
		...
		$newGroup = array('name' => htmlspecialchars($group['grouptitle']),
...
function install_product($xml, $allow_overwrite = false, $verbose = true)
{
	...
	if (isset($arr['options']['settinggroup']) AND is_array($arr['options']['settinggroup']))
	{
		...
		foreach ($settinggroups AS $group)
		{
			...
			$check = $assertor->assertQuery('settinggroup', array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_SELECT,
			...
			if ($check->valid())
			{
				...
				{
					$assertor->assertQuery('settinggroup', array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_UPDATE,
						'grouptitle' => $group['name'],
						'displayorder' => $group['displayorder'],
			...
			{
				/*insert query*/
				$assertor->assertQuery('settinggroup', array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_INSERTIGNORE,
					'grouptitle' => $group['name'],
					'displayorder' => $group['displayorder'],
					'volatile' => 1,
					'product' => $info['productid'],
			...
			foreach ($settings AS $setting)
			{
				...
				$setting_bits[] = array(
					...
					'product' => $info['productid'],
					'adminperm' => (empty($setting['adminperm']) ? '' : $setting['adminperm'])
				);
