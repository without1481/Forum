...
// NOTE: '&' . vB::getCurrentSession()->get('sessionurl') will be AUTOMATICALLY added to the URL - do not add to your link!
	...
	$url .= (empty($url_query) ? '?' : '&amp;');
	$options[] = "\t\t<a class=\"navlink\" href=\"$prefix/$url$sessionlink\">$title</a>\n";
}
	...
	if (VB_AREA == 'AdminCP')
	{
		...
		<tr>
			...
			<td align=\"" . vB_Template_Runtime::fetchStyleVar('right') . "\">
				<a href=\"admincp/index.php?" . vB::getCurrentSession()->get('sessionurl') . "do=buildnavprefs&amp;nojs=" . $vbulletin->GPC['nojs'] . "&amp;prefs=$navlinks&amp;dowhat=$dowhat&amp;id={$nav_file}_$groupid[$nav_file]#grp{$nav_file}_$groupid[$nav_file]\" target=\"_self\"

					oncontextmenu=\"toggle_group('{$nav_file}_$groupid[$nav_file]'); save_group_prefs('{$nav_file}_$groupid[$nav_file]'); return false\"
				><img class=\"acp-nav-arrow\" src=\"core/cpstyles/" . $vb_options['cpstylefolder'] . "/cp_$dowhat" . '_' . vB_Template_Runtime::fetchStyleVar('textdirection') . '.' . $vb_options['cpstyleimageext'] . "\" title=\"$tooltip\" id=\"button_{$nav_file}_$groupid[$nav_file]\" alt=\"+\" border=\"0\" /></a>
			</td>
		</tr>
		</table>";
		$_NAV .= "
...
function print_nav_panel()
{
	...
	if ($vbulletin->GPC['nojs'])
	{
		$controls .= "<div class=\"acp-nav-controls\">
			<div>
				<a class=\"nav-left\" href=\"admincp/index.php?" . vB::getCurrentSession()->get('sessionurl') . "do=navprefs&amp;nojs=" . $vbulletin->GPC['nojs'] . "&amp;groups=$groups&amp;expand=1&amp;$numgroups\" onclick=\"expand_all_groups(1); return false;\" target=\"_self\">" . $vbphrase['expand_all'] . "</a>
				<a class=\"nav-right\" href=\"admincp/index.php?" . vB::getCurrentSession()->get('sessionurl') . "do=navprefs&amp;nojs=" . $vbulletin->GPC['nojs'] . "&amp;groups=$groups&amp;expand=0&amp;$numgroups\" onclick=\"expand_all_groups(0); return false;\" target=\"_self\">" . $vbphrase['collapse_all'] . "</a>
			</div>
		</div>";
	}
	else
...
?>
	...
	function open_close_group(group, doOpen)
	{
		...
		if (doOpen)
		{
			...
			curbtn.src = "core/cpstyles/<?php echo $vbulletin->options['cpstylefolder']; ?>/cp_collapse.<?php echo $vbulletin->options['cpstyleimageext']; ?>";
		...
		else
		{
			...
			curbtn.src = "core/cpstyles/<?php echo $vbulletin->options['cpstylefolder']; ?>/cp_expand_<?php echo vB_Template_Runtime::fetchStyleVar('textdirection') . '.' . $vbulletin->options['cpstyleimageext']; ?>";
	...
	function save_group_prefs(groupid)
	{
		...
		vBRedirect("index.php?<?php echo vB::getCurrentSession()->get('sessionurl_js'); ?>do=savenavprefs&nojs=<?php echo $vbulletin->GPC['nojs']; ?>&navprefs=" + opengroups.join(",") + "#grp" + groupid);
	}
