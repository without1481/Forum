...
/**
* get the template groups
* $only is the (badly named) list of template groups
*/
global $only;
$only = build_template_groups($vbphrase);
...
{
	if (preg_match('#@import url\("(' . vB_Api::instanceInternal('style')->fetchCssLocation(). '/style-\w{8}-0*' . $styleid . '\.css)"\);#siU', $csscontents, $match))
	{
...
function write_style_css_directory($styleid, $parentlist, $dir = 'ltr')
{
	...
	$set = vB::getDbAssertor()->assertQuery('template_fetch_css_templates', array('styleidlist' => $parentlistarr));
	...
	//the ltr/rtl logic in sync with the css.php handling
...
function build_style($styleid, $title = '', $actions = array(), $parentlist = '', $indent = '', $resetcache = false, $printInfo = true)
{
	...
	if ($styleid != -1)
	{
		...
		vB_Library::instance('Style')->internalCacheClear($styleid);
		if ($printInfo AND (VB_AREA != 'Upgrade' AND VB_AREA != 'Install'))
		{
...
function rewrite_css_file_url($url, $delimiter = '')
{
	...
	if ($iswritable === null)
	{
		$iswritable = is_writable(DIR . vB_Api::instanceInternal('style')->fetchCssLocation());
	}
...
function print_style($styleid, $style = array())
{
	...
	$styleIsReadonly = !vB_Library::instance('style')->checkStyleReadProtection($styleid, $style);
	$showReadonlyMarking = !vB_Library::instance('style')->checkStyleReadProtection($styleid, $style, true);
	...
	//serious hack time -- we should htmlencode the clicklink to avoid XSS, but the routecode
	//pre-escapes some & annoyingly.  The real fix is remove that from the route code and make the
	//caller responsible for html escapes (its not entirely clear that we always want to escape in
	//all contexts).  This hack will causes things to fail if the parameters contain an "&" that needs
	//escaping, but those wouldn't work before we added the htmlentites call below that caused the
	//breakage anyway, so we can be pretty sure it doesn't happen in the current code.
	$onclicklink = str_replace('&amp;', '&', $onclicklink);
	...
	<tr>
		...
		"</label><a href=\"$forumhome_url\" target=\"_blank\" title=\"$vbphrase[view_your_forum_using_this_style]\">$title</a></td>
	...
	if ($userContext->hasAdminPermission('canadmintemplates') AND !$styleIsReadonly)
	{
		echo"
	...
	if ($userContext->hasAdminPermission('canadminstyles') AND !$styleIsReadonly)
	{
		echo  "<optgroup label=\"" . $vbphrase['edit_fonts_colors_etc'] . "\"> ";
	}
	if ($userContext->hasAdminPermission('canadmintemplates') AND !$styleIsReadonly)
	{
		echo "<option value=\"css_templates\">" . $vbphrase['common_templates'] . "</option> \n";
	}
	...
	if ($userContext->hasAdminPermission('canadminstyles'))
	{
		if (!$styleIsReadonly)
		{
			echo "	<option value=\"stylevar\">$vbphrase[stylevareditor]</option>
		...
		echo "			<optgroup label=\"" . $vbphrase['edit_style_options'] . "\">
	...
	{
		echo "<option value=\"template_download\">" . $vbphrase['download'] . "</option>\n";
	}

	if ($userContext->hasAdminPermission('canadminstyles'))
	{
		echo '<option value="template_delete" class="col-c">' . $vbphrase['delete_style'] ."</option>\n";
	}
	...
	echo "</optgroup>
			...
			onclick=\"vBRedirect('" . htmlspecialchars($onclicklink) . "');\" />
			&nbsp;
		</td>
	</tr>
	</table>
				...
				else
				{
					foreach($only AS $thisgroup => $display)
					{
						...
						if (isTemplateInGroup($template['title'], $thisgroup))
						{
							...
							{
								echo "<optgroup label=\"\">\n";
								echo "\t<option class=\"templategroup\" value=\"[]\"" .
									(($group == $thisgroup AND empty($vbulletin->GPC['templateid']))? ' selected="selected"' : '') . ">" .
	...
	*/
	...
	if (empty($safe_functions))
	{
		$safe_functions = array(
			...
			//probably not useful but included for consistancy with other "safe functions" list
			'sprintf',
		);
...
function construct_color_picker($size = 12, $display = 'none')
{
	...
	<tr>
		<td><button type=\"button\" onclick=\"col_click('transparent'); return false\"><img src=\"core/cpstyles/" . $vbulletin->options['cpstylefolder'] . "/colorpicker_transparent.gif\" title=\"'transparent'\" alt=\"\" /></button></td>
		<td>
		...
		<td style=\"white-space:nowrap\">
			...
			<button type=\"button\" onclick=\"switch_color_picker(1); return false\"><img src=\"core/cpstyles/" . $vbulletin->options['cpstylefolder'] . "/colorpicker_toggle.gif\" alt=\"\" /></button>
			<button type=\"button\" onclick=\"close_color_picker(); return false\"><img src=\"core/cpstyles/" . $vbulletin->options['cpstylefolder'] . "/colorpicker_close.gif\" alt=\"\" /></button>
		</td>
	</tr>
	</table>
...
)
{
	...
	$template_groups = build_template_groups($vbphrase);
...
*
* @param	array 	Style phrases
*
* @return	array	Template group names
*/
function build_template_groups($vbphrase)
{
	return array
	(
