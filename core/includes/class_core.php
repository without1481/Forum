...
define('FILE_VERSION', '5.3.4'); // this should match installsteps.php
if (!defined('SIMPLE_VERSION')) define('SIMPLE_VERSION', '534'); // see vB_Datastore::check_options()
define('YUI_VERSION', '2.7.0'); // define the YUI version we bundle
define('JQUERY_VERSION', '2.1.4'); // define the jQuery version we use
...
*
* @package	vBulletin
* @version	$Revision: 96147 $
* @date		$Date: 2017-11-15 08:53:54 -0800 (Wed, 15 Nov 2017) $
*/
	...
	*/
	function fetch_url()
	{
		...
		if (empty($url) OR $url == $scriptpath)
		{
			...
			try
			{
				$url = vB5_Route::buildUrl('home|fullurl');
			}
			catch (Exception $e)
			{
				$url = '';
...
abstract class vB_Template_Data extends vB_Template
{
	...
	public function register($name, $value, $overwrite = true)
	{
		...
		{
			$value = $this->forceRenderPhrases($value);
		}
	...
	protected function forceRenderPhrases($value)
	{
		if (is_array($value))
		{
			foreach ($value AS &$el)
			{
				$el = $this->forceRenderPhrases($el);
			}
		}

		if ($value instanceof vB_Phrase)
		{
			$value = (string)$value;
		}
