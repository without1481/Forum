...
define('CVS_REVISION', '$RCSfile$ - $Revision: 94014 $');
...
if ($_REQUEST['do'] == 'add' OR $_REQUEST['do'] == 'edit' OR $_REQUEST['do'] == 'editglobal')
{
	...
	if ($_REQUEST['do'] == 'editglobal')
	{
		...
		print_form_header('admincp/moderator', 'update');
	...
	else
	{
		...
		// delete link
		print_form_header('admincp/moderator', 'remove');
...
if ($_POST['do'] == 'update')
{
	...
	//reset the list of moderators to be notified on new posts
	vB_Cache::instance(vB_Cache::CACHE_STD)->event('vB_ModPostNotifyChg');
	...
	foreach ($modnames AS $name)
	{
		...
		$moddata = new vB_DataManager_Moderator(vB_DataManager_Constants::ERRTYPE_CP);
...
if ($_REQUEST['do'] == 'showlist')
{
	...
	if (count($supergroups))
	{
		foreach ($supergroups AS $supergroup)
		{
			...
			echo "\n\t<li><b><a href=\"admincp/user.php?" . vB::getCurrentSession()->get('sessionurl') .
				"do=edit&u=$supergroup[userid]\">$supergroup[username]</a></b><span class=\"smallfont\"> (" .
				construct_link_code($vbphrase['edit_permissions'], "moderator.php?" . vB::getCurrentSession()->get('sessionurl') .
				"do=editglobal&amp;u=$supergroup[userid]") . ") - " . $vbphrase['last_online'] . " <span class=\"$onlinecolor\">" .
				$lastonline . "</span></span></li>\n";
		}
	}
	else
	{
	...
	if (count($moderators))
	{
		...
		foreach ($moderators AS $moderator)
		{
			if ($curmod != $moderator['userid'])
			{
				...
				echo "\n\t<ul>\n\t<li><b><a href=\"admincp/user.php?" . vB::getCurrentSession()->get('sessionurl') .
					"do=edit&amp;u=$moderator[userid]&amp;redir=showlist\">$moderator[username]</a></b><span class=\"smallfont\"> - " .
					$vbphrase['last_online'] . " <span class=\"$onlinecolor\">" . $lastonline . "</span></span>\n";

				echo "\n\t\t<ul>$vbphrase[channels] <span class=\"smallfont\">(" .
					construct_link_code($vbphrase['remove_moderator_from_all_channels'], "moderator.php?" . vB::getCurrentSession()->get('sessionurl') .
					"do=removeall&amp;u=$moderator[userid]") . ")</span>\n\t<ul>\n";
			}
			echo "\t\t\t<li><a href=\"" . vB5_Route::buildUrl($moderator['routeid'].'|bburl') . "\" target=\"_blank\">$moderator[htmltitle]</a>\n" .
				"\t\t\t\t<span class=\"smallfont\">(" . construct_link_code($vbphrase['edit'], "moderator.php?" . vB::getCurrentSession()->get('sessionurl') .
				"do=edit&moderatorid=$moderator[moderatorid]&amp;redir=showlist") .
				construct_link_code($vbphrase['remove'], "moderator.php?" . vB::getCurrentSession()->get('sessionurl') .
				"do=remove&moderatorid=$moderator[moderatorid]") . ")</span>\n".
				"\t\t\t</li><br />\n";
		}
		echo "\t\t</ul>\n\t\t</ul>\n\t</li>\n\t</ul>\n";
	}
	...
	if (count($channels))
	{
		foreach ($channels AS $channel)
		{
			...
			if ($curnode != $channel['nodeid'])
			{
				...
				$channelurl = vB5_Route::buildUrl($channel['routeid'] . '|fullurl', $channel);
				echo "\n\t<ul>\n\t<li><b><a href=\"$channelurl\">$channel[htmltitle]</a></b>\n";
			...
			echo "\t\t\t<li><a href=\"admincp/user.php?do=edit&amp;u=$channel[userid]\" target=\"_blank\">$channel[username]</a>" .
				"\t\t\t\t<span class=\"smallfont\">(" .
				construct_link_code($vbphrase['edit'], "moderator.php?do=edit&moderatorid=$channel[moderatorid]&redir=showmods") .
				construct_link_code($vbphrase['remove'], "moderator.php?do=remove&moderatorid=$channel[moderatorid]&redir=showmods") . ")" .
				" - " . $vbphrase['last_online'] . " <span class=\"$onlinecolor\">" . $lastonline . "</span></span>\n" .
				"\t\t\t</li><br />\n";
		}
		echo "\t\t</ul>\n\t\t</ul>\n\t</li>\n\t</ul>\n";
...
if ($_REQUEST['do'] == 'removeall')
{
	...
	print_form_header('admincp/moderator', 'killall', 0, 1, '', '75%');
