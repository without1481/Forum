...
define('CVS_REVISION', '$RCSfile$ - $Revision: 95270 $');
...
if ($_REQUEST['do'] == 'duplicate')
{
	...
	print_form_header('admincp/prefix', 'doduplicate');
...
if ($_REQUEST['do'] == 'permissions')
{
	...
	{
		$prefix_html[] = '<a href="admincp/prefix.php?do=editprefix&amp;prefixid=' . htmlspecialchars_uni($prefixid) . '">' . htmlspecialchars_uni($vbphrase["prefix_{$prefixid}_title_plain"]) . '</a>';
	}
	...
	print_form_header('admincp/prefix', 'savepermissions');
...
if ($_POST['do'] == 'killprefix')
{
	...
	$prefix = vB::getDbAssertor()->getRow('vBForum:prefix', array('prefixid' => $vbulletin->GPC['prefixid']));
...
if ($_REQUEST['do'] == 'addprefix' OR $_REQUEST['do'] == 'editprefix')
{
	...
	print_form_header('admincp/prefix', 'insertprefix');
...
if ($_REQUEST['do'] == 'addset' OR $_REQUEST['do'] == 'editset')
{
	...
	print_form_header('admincp/prefix', 'insertset');
...
if ($_REQUEST['do'] == 'list')
{
	...
	print_form_header('admincp/prefix', 'displayorder');
	...
	print_cells_row(array(
		'<input type="image" style="width: 1px; height: 1px;" src="images/clear.gif" />' .
		...
		'<input class="button" type="button" onclick="vBRedirect(\'admincp/prefix.php?do=addset\');" value="'. $vbphrase['add_prefix_set'] . '" />'
	), false, 'tfoot');
