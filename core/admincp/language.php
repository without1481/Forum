...
define('CVS_REVISION', '$RCSfile$ - $Revision: 95370 $');
...
if ($_POST['do'] == 'upload')
{
	...
	$args['do'] = 'rebuild';
	$args['goto'] = 'admincp/language.php?' . vB::getCurrentSession()->get('sessionurl');
...
if ($_REQUEST['do'] == 'edit')
{
	...
	if ($vbulletin->GPC['dolanguageid'] == -2)
	{
		...
		print_cp_redirect2('phrase', $args, 0, 'admincp');
	}
	...
	echo '
		...
		<tr>
			...
			<td class="thead" rowspan="2"><!--' . $vbphrase['page_gcpglobal'] . ':-->
				...
				' . ($showprev ? ' <input type="submit" class="button" name="prev" value="&laquo; ' . $vbphrase['prev'] . '" />' : '') . '
				' . ($shownext ? ' <input type="submit" class="button" name="next" value="' . $vbphrase['next'] . ' &raquo;" />' : '') . '
			</td>
...
if ($_REQUEST['do'] == 'modify')
{
	...
	*/
	...
	print_form_header('admincp/language', 'add');
	construct_hidden_code('goto', "admincp/language.php?" . vB::getCurrentSession()->get('sessionurl'));
	...
	print_description_row(
		...
		construct_link_code($vbphrase['view_quickref_glanguage'], "javascript:js_open_phrase_ref(0,0);", false, '', false, false) .
		construct_link_code($vbphrase['rebuild_all_languages'], "language.php?" . vB::getCurrentSession()->get('sessionurl') . "do=rebuild&amp;goto=admincp/language.php?" . vB::getCurrentSession()->get('sessionurl'))
	...
	print_table_footer(4, '
		...
		<input type="button" class="button" value="' . $vbphrase['download_upload_language'] . '" tabindex="1" onclick="vBRedirect(\'admincp/language.php?do=files\');" />
	');
