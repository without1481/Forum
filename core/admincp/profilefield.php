...
define('CVS_REVISION', '$RCSfile$ - $Revision: 93702 $');
...
if ($_REQUEST['do'] == 'addcat' OR $_REQUEST['do'] == 'editcat')
{
	...
	print_form_header('admincp/profilefield', 'updatecat');
...
if ($_POST['do'] == 'update')
{
	...
	$vbulletin->GPC['profilefield']['form'] = 0;
...
if ($_REQUEST['do'] == 'modifycheckbox')
{
	...
	if ($boxdata['data'] != '')
	{
		...
		foreach ($data AS $index => $value)
		{
			...
			if ($index != 1)
			{
				$imageurl = $vbulletin->options['bburl'] . "/cpstyles/" . $vbulletin->options['cpstylefolder'] . "/move_up.gif";
				$moveup = "<a href=\"admincp/profilefield.php?profilefieldid=" . $vbulletin->GPC['profilefieldid'] .
					"&do=movecheckbox&direction=up&id=$index\"><img src=\"$imageurl\" border=\"0\" /></a> ";
			}
			else
			{
				$moveup = '<img src="images/clear.gif" width="11" border="0" alt="" /> ';
			}
			if ($index != sizeof($data))
			{
				$imageurl = $vbulletin->options['bburl'] . "/cpstyles/" . $vbulletin->options['cpstylefolder'] . "/move_down.gif";
				$movedown = "<a href=\"admincp/profilefield.php?profilefieldid=" . $vbulletin->GPC['profilefieldid'] .
					"&do=movecheckbox&direction=down&id=$index\"><img src=\"$imageurl\" border=\"0\" /></a> ";
			}
			else
			{
				unset($movedown);
	...
	if (sizeof($data) < 31)
	{
		print_form_header('admincp/profilefield', 'addcheckbox');
		construct_hidden_code('profilefieldid', $vbulletin->GPC['profilefieldid']);
