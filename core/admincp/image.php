...
define('CVS_REVISION', '$RCSfile$ - $Revision: 90620 $');
...
if ((($itemtype == 'smilie') OR ($itemtype == 'post_icon')) AND !vB::getUserContext()->hasAdminPermission('cansetserverconfig'))
{
	print_cp_no_permission();
}
...
if ($_REQUEST['do'] == 'modify')
{
	...
	if ($categories AND $categories->valid())
	{
		print_form_header('admincp/image', 'docategorydisplayorder');
		construct_hidden_code('table', $vbulletin->GPC['table']);
		...
		{
			$cell = array();
			$cell[] = "<a href=\"admincp/image.php?" . vB::getCurrentSession()->get('sessionurl') . "do=viewimages&table=" . $vbulletin->GPC['table'] . "&imagecategoryid=$category[imagecategoryid]\">$category[title]</a>";
