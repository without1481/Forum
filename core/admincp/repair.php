...
define('CVS_REVISION', '$RCSfile$ - $Revision: 92588 $');
...
if (!can_administer('canuseallmaintenance'))
{
...
if ($_POST['do'] == 'dorepair')
{
	...
	print_form_header('admincp/repair', 'dorepair');
...
if ($_REQUEST['do'] == 'list')
{
	print_form_header('admincp/repair', 'dorepair', 0, 1, 'cpform');
	...
	print_form_header('admincp/repair', 'fixunique', 0, 1, 'bla');
...
if ($_REQUEST['do'] == 'fixunique')
{
	...
	function fix_unique_index($tableid)
	{
		...
		else
		{
			...
			$createindexquery = "ALTER TABLE " . TABLE_PREFIX . "$table[name] ADD UNIQUE KEY $table[keyname] ($fields)";
	...
	{
		if (preg_match('#unique key (\w+)\s*\(([\w, ]+)\)#siU', $query, $regs))
		{
	...
	if ($nexttableid >= 0)
	{
		print_form_header('admincp/repair', 'fixunique', 0, 1, 'cpform', '25%');
	...
	{
		print_form_header('admincp/repair', '', 0, 1, 'cpform', '65%');
		print_table_header($vbphrase['fix_unique_indexes']);
