...
define('CVS_REVISION', '$RCSfile$ - $Revision: 93523 $');
...
if ($_REQUEST['do'] == 'edit')
{
	...
	if ($nodeid > 1)
	{
		$inherit = '<label for="uug_1"><input type="radio" name="inherit" value="1" id="inherit_1" onclick="this.form.reset(); this.checked=true;"' .
			(empty($permissionid) ? ' checked="checked"': '') . ' />' . $vbphrase['inherit_channel_permission'] . '</label>';

		$custom = '	<label for="uug_0"><input type="radio" name="inherit" value="0" id="inherit_0"' .
...
if ($_REQUEST['do'] == 'quickedit')
{
	...
	print_cells_row(array(
		...
		"<a href=\"admincp/forumpermission.php?" . vB::getCurrentSession()->get('sessionurl') . "do=quickedit&amp;orderby=channel\" title=\"" . $vbphrase['order_by_channel'] . "\">" . $vbphrase['channel'] . "</a>",
		"<a href=\"admincp/forumpermission.php?" . vB::getCurrentSession()->get('sessionurl') . "do=quickedit&amp;orderby=usergroup\" title=\"" . $vbphrase['order_by_usergroup'] . "\">" . $vbphrase['usergroup'] . "</a>",
		$vbphrase['controls']
	), 1);
...
function print_channels($permissions, $inheritance = array(), $channels = false, $indent = '	')
{
	...
	foreach ($channels AS $nodeid => $node)
	{
		...
		if (!defined('ONLYID') OR $nodeid == ONLYID)
		{
			...
			echo "$indent<li><b><a name=\"node$nodeid\" href=\"admincp/forum.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;n=$nodeid\">$node[htmltitle]</a></b>";
			if ($node['parentid'] != 0)
			{
