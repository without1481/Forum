...
define('CVS_REVISION', '$RCSfile$ - $Revision: 95653 $');
...
{
	$cssfile = vB_Api::instanceInternal('style')->fetchCssLocation() . str_pad($vbulletin->options['styleid'], 5, '0', STR_PAD_LEFT) . $vbulletin->stylevars['textdirection']['string'][0] . '/' . 'stylegenerator.css';
}
else
{
	$cssfile = 'css.php?styleid=' . $vb5_options['styleid'] . '&amp;langid=' . LANGUAGEID . '&amp;d=' . $stylestuff['dateline'] . '&amp;td=' . $vbulletin->stylevars['textdirection']['string'] . '&amp;sheet=stylegenerator.css';
}
...
if ($_REQUEST['do'] == 'stylegenerator' AND !(is_browser('ie') AND !is_browser('ie', 7)))
{
	...
	$extraheaders =
		...
		<script type="text/javascript" src="core/clientscript/jquery/jquery-1.3.min.js?v=' . SIMPLE_VERSION . '"></script>
		<script type="text/javascript" src="core/clientscript/jquery/jquery.styledselect.js?v=' . SIMPLE_VERSION . '"></script>
		<script type="text/javascript" src="core/clientscript/jquery/jquery.floatbox.js?v=' . SIMPLE_VERSION . '"></script>
		<script type="text/javascript" src="core/clientscript/jquery/jquery.tooltip.js?v=' . SIMPLE_VERSION . '"></script>
		<script type="text/javascript" src="core/clientscript/jquery/jquery.droppy.js?v=' . SIMPLE_VERSION . '"></script>
		<script type="text/javascript" src="core/clientscript/vbulletin_stylegeneratorcolor.js?v=' . SIMPLE_VERSION . '"></script>
		<script type="text/javascript" src="core/clientscript/vbulletin_stylegenerator.js?v=' . SIMPLE_VERSION . '"></script>';
}
...
if ($_REQUEST['do'] == 'files')
{
	...
	', '', 'top', 'mode');
	print_checkbox_row($vbphrase['remove_unique_identifier'], 'remove_guid', false, 1, $vbphrase['remove_guid']);
...
if ($_REQUEST['do'] == 'add')
{
	...
	print_input_row(
		...
		'title',
		$vbulletin->GPC['title']);
	print_checkbox_row($vbphrase['textonly'] . '<dfn>' . $vbphrase['textonly_desc'] . '</dfn>',
		'textonly', false);
...
if ($_REQUEST['do'] == 'edit')
{
	...
	print_label_row($vbphrase['style'], "<a href=\"admincp/$backlink\" title=\"" . $vbphrase['edit_templates'] . "\"><b>$template[style]</b></a>");
	print_input_row(
...
if ($_REQUEST['do'] == 'modify')
{
	...
	$stylecache = vB_Library::instance('Style')->fetchStyles(false, false, array('themes' => true));
...
<tr>
	...
	if (vB::getUserContext()->hasAdminPermission('canadmintemplates'))
	{
		echo "		<input type=\"button\" class=\"button\" tabindex=\"1\" value=\"" . $vbphrase['search_in_templates_gstyle'] .
			"\" onclick=\"vBRedirect('admincp/template.php?" . vB::getCurrentSession()->get('sessionurl_js') . "do=search');\" />\n";
	}
?>
	</td>
</tr>
