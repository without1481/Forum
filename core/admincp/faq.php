...
if ($_REQUEST['do'] == 'updatefaq')
{
	...
	print_form_header('admincp/faq', 'doupdatefaq');
...
if ($_POST['do'] == 'kill')
{
	...
	if (defined('DEV_AUTOEXPORT') AND DEV_AUTOEXPORT)
	{
		...
		{
			$set = $assertor->assertQuery('vBForum:getDistinctProduct', array('phraseDeleteNamesSql' => $phraseDeleteNamesSql));
...
if ($_REQUEST['do'] == 'edit' OR $_REQUEST['do'] == 'add')
{
	...
	else
	{
		...
		print_form_header('admincp/faq', 'insert', 0, 1, 'cpform" onsubmit="return js_check_shortname(this, ' . iif($vb5_config['Misc']['debug'], 'false', 'true') . ');');
...
if ($_POST['do'] == 'updateorder')
{
	...
	{
		require_once(DIR . '/includes/functions_filesystemxml.php');
		$products = $assertor->assertQuery('vBForum:getDistinctProductFAQ', array('faqnames'=>implode(', ', $faqnames)));
...
if ($_REQUEST['do'] == 'modify')
{
	...
	$nav = "<a href=\"admincp/faq.php?" . vB::getCurrentSession()->get('sessionurl') . "\">$vbphrase[faq]</a>";
	if (!empty($parents))
	{
		$i = 1;
		foreach($parents AS $link => $name)
		{
			$nav .= '<br />' . str_repeat('&nbsp; &nbsp; ', $i) . iif(empty($link), $name, "<a href=\"admincp/$link\">$name</a>");
			$i ++;
		}
		$nav .= '
