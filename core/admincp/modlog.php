...
define('CVS_REVISION', '$RCSfile$ - $Revision: 88589 $');
...
if ($_REQUEST['do'] == 'view')
{
	...
	{
		$baseUrl = "admincp/modlog.php?" . vB::getCurrentSession()->get('sessionurl');
		$query = array(
			'do' => 'view',
		...
		foreach ($logs as $log)
		{
			...
			$cell[] = "<a href=\"admincp/user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&u=$log[userid]\"><b>$log[username]</b></a>";
			...
			if ($log['type'])
			{
				...
				else
				{
					...
					{
						$name = '<a href="admincp/user.php?' . vB::getCurrentSession()->get('sessionurl') . 'do=edit&u=' . $logdata['userid'] . '">' . $logdata['username'] . '</a>';
					...
					if ($log['routeid'])
					{
						$titleurl = vB5_Route::buildUrl($log['routeid'] . '|fullurl', array('nodeid' => $log['nodeid'], 'title' => $title, 'innerPost' => $log['nodeid']), array(), '#post' . $log['nodeid']);
			...
			$cell[] = '<span class="smallfont">' . iif($log['ipaddress'], "<a href=\"admincp/usertools.php?" . vB::getCurrentSession()->get('sessionurl') . "do=gethost&ip=$log[ipaddress]\">$log[ipaddress]</a>", '&nbsp;') . '</span>';
