...
define('CVS_REVISION', '$RCSfile$ - $Revision: 95513 $');
...
if ($_POST['do'] == 'update')
{
	...
	$args['insertedadmin'] = vB::getUserContext($userid)->isAdministrator();
...
if ($_REQUEST['do'] == 'modify')
{
	...
	if ($vbulletin->GPC['userid'])
	{
		...
		print_description_row(
			construct_link_code($vbphrase['view_profile'], "user.php?do=edit&amp;u=" . $vbulletin->GPC['userid']) .
			iif($vbulletin->GPC['insertedadmin'], '<br />' . construct_link_code('<span style="color:red;"><strong>' . $vbphrase['update_or_add_administration_permissions'] .
				'</strong></span>', "adminpermissions.php?do=edit&amp;u=" . $vbulletin->GPC['userid']))
		);
		print_table_footer();
	}
...
if ($_REQUEST['do'] == 'prune')
{
	...
	if ($_POST['do'] == 'do_emailpassword')
	{
		...
		$success = vB_Library::instance('user')->sendPasswordEmail($userid, $email);
		if ($success)
		{
		...
		else
		{
			...
			standard_error(fetch_error('invalidemail', vB5_Route::buildUrl('contact-us|fullurl')));
		}
	}
	...
	if ($_REQUEST['do'] == 'emailcode')
	{
		...
		$users = $assertor->assertQuery('vBAdmincp:getUserActivation', array('userid' => $userid));
			...
			foreach ($users as $user)
			{
				...
				{ // only do it if the user is in the correct usergroup
					...
					else
					{
						...
						$assertor->assertQuery('useractivation', array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_UPDATE,
							'dateline' => $timeNow, 'activationid' => $user['activationid'],
							vB_dB_Query::CONDITIONS_KEY => array('userid' => $user['userid'], 'type' => 0)));
					}

					$vboptions = vB::getDatastore()->getValue('options');
					$maildata = vB_Api::instanceInternal('phrase')->fetchEmailPhrases('activateaccount',
						array($user['username'], $vboptions['bbtitle'], $vboptions['frontendurl'], $user['userid'],
							$user['activationid'], $vboptions['webmasteremail']), array($user['username']), $user['languageid']);
