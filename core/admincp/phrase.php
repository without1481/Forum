...
define('CVS_REVISION', '$RCSfile$ - $Revision: 94908 $');
...
if ($_REQUEST['do'] == 'findorphans')
{
	...
	foreach ($orphans AS $key => $languageids)
	{
		...
		foreach ($languages AS $language)
		{
			...
			$cell[] = "<img src=\"" . $datastore->getOption('bburl') . "/cpstyles/" .
				$datastore->getOption('cpstylefolder') . "/cp_tick_$yesno.gif\" alt=\"\" />";
		}
...
if ($_POST['do'] == 'doreplace')
{
	...
	$args['do'] = 'rebuild';
	$args['goto'] = "admincp/phrase.php?do=search";
	exec_header_redirect2('language', $args);
}
...
if ($_REQUEST['do'] == 'edit')
{
	...
	else
	{
		...
		if (($phrase['languageid'] == 0 OR $vb5_config['Misc']['debug']) AND !$vbulletin->GPC['t'])
		{
			...
			print_label_row(
				$vbphrase['text_gcpglobal'] . $resizer,
				"<textarea name=\"text[0]\" id=\"default_phrase\" rows=\"4\" cols=\"50\" wrap=\"virtual\" tabindex=\"1\" dir=\"ltr\"" . iif($vb5_config['Misc']['debug'], ' title="name=&quot;text[0]&quot;"') . " $autofocus>" . htmlspecialchars_uni($phrase['text']) . "</textarea>",
				'', 'top', 'text[0]'
			);
			$autofocus = '';
		}
		...
		foreach($languages AS $lang)
		{
			...
			$label = construct_phrase($vbphrase['x_translation'], "<b>{$lang['title']}</b>") .
				" <dfn>($vbphrase[optional])</dfn><br /><input type=\"button\" class=\"button\" value=\"$vbphrase[copy_default_text]\" tabindex=\"1\" onclick=\"copy_default_text({$lang['languageid']});\" />" . $resizer;

			$control = '<textarea name="text[' . $lang['languageid'] . ']" id="text_' . $lang['languageid'] . '" ' .
				'rows="5" cols="60" tabindex="1" wrap="virtual" dir="' . $lang['direction'] . '" ' . $autofocus .'>' .
				htmlspecialchars_uni($text["{$lang['languageid']}"]) . '</textarea>';

			print_label_row($label, $control);
			print_description_row('<img src="images/clear.gif" width="1" height="1" alt="" />', 0, 2, 'thead');
			$autofocus='';
		}
...
/*if (empty($vbulletin->GPC['showpt']))
	...
	print_table_footer($colspan, "
		<input type=\"button\" class=\"button\" value=\"" . $vbphrase['search_in_phrases_glanguage'] .
			"\" tabindex=\"1\" onclick=\"vBRedirect('admincp/phrase.php?" .  vB::getCurrentSession()->get('sessionurl') . "&amp;do=search');\" />
		&nbsp; &nbsp;
		<input type=\"button\" class=\"button\" value=\"" . $vbphrase['add_new_phrase'] .
			"\" tabindex=\"1\" onclick=\"vBRedirect('admincp/phrase.php?" . vB::getCurrentSession()->get('sessionurl') .
		...
		<input type=\"button\" class=\"button\" value=\"" . $vbphrase['find_orphan_phrases'] .
			"\" tabindex=\"1\" onclick=\"vBRedirect('admincp/phrase.php?" . vB::getCurrentSession()->get('sessionurl') . "do=findorphans');\" />
	");
