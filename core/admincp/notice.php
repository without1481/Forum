...
define('CVS_REVISION', '$RCSfile$ - $Revision: 92054 $');
...
if ($_REQUEST['do'] == 'edit' OR $_REQUEST['do'] == 'add')
{
	...
	print_textarea_row($vbphrase['notice_html'] . '<dfn>' . $vbphrase['notice_html_description'] . '</dfn>' . ($vbulletin->GPC['noticeid'] ? '<div class="smallfont" style="margin-top:6px"><a href="admincp/phrase.php?do=edit&amp;fieldname=global&amp;phraseid=' . $notice['html_phraseid'] . '" target="translate">' . $vbphrase['translations'] . '</a></div>' : ''), 'html', $notice['html'], 8, 60, true, false);
...
if ($_REQUEST['do'] == 'modify')
{
	...
	if ($notice_count)
	{
		print_description_row('<label><input type="checkbox" id="allbox" checked="checked" />' . $vbphrase['toggle_active_status_for_all'] . '</label><input type="image" value="" src="images/clear.gif" name="normalsubmit" />', false, 2, 'thead checkbox-in-thead');
		foreach ($notice_result AS $notice)
		{
			print_label_row(
				'<a href="admincp/notice.php?' . vB::getCurrentSession()->get('sessionurl') . 'do=edit&amp;noticeid=' . $notice['noticeid'] . '" title="' . $vbphrase['edit_notice'] . '">' . $notice['title'] . '</a>',
