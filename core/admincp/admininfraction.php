...
define('CVS_REVISION', '$RCSfile$ - $Revision: 94710 $');
...
if ($_REQUEST['do'] == 'dolist')
{
	...
	if ($infractions AND $infractions->valid())
	{
		if ($vbulletin->GPC['pagenumber'] != 1)
		{
			...
			$url = htmlspecialchars("admincp/admininfraction.php?do=dolist" . $args . $vbulletin->GPC['orderby'] . "&page=1");
			$firstpage = "<input type=\"button\" class=\"button\" tabindex=\"1\" value=\"&laquo; " . $vbphrase['first_page'] .
				"\" onclick=\"vBRedirect('$url');\">";
			$url = htmlspecialchars("admincp/admininfraction.php?do=dolist" . $args . $vbulletin->GPC['orderby'] . "&page=$prv");
		...
		if ($vbulletin->GPC['pagenumber'] != $totalpages)
		{
			...
			$url = htmlspecialchars("admincp/admininfraction.php?do=dolist" . $args . $vbulletin->GPC['orderby'] . "&page=$nxt");
			$nextpage = "<input type=\"button\" class=\"button\" tabindex=\"1\" value=\"" . $vbphrase['next_page'] . " &gt;\" onclick=\"vBRedirect('$url');\">";
			$url = htmlspecialchars("admincp/admininfraction.php?do=dolist" . $args . $vbulletin->GPC['orderby'] . "&page=$totalpages");
		...
		$headings = array();
		$headings[] = "<a href=\"admincp/admininfraction.php?do=dolist" . $args . "\" title=\"" . $vbphrase['order_by_username'] . "\">" . $vbphrase['user_name'] . "</a>";
		$headings[] = "<a href=\"admincp/admininfraction.php?do=dolist" . $args . "leftby_username\" title=\"" . $vbphrase['order_by_username'] . "\">" . $vbphrase['left_by'] . "</a>";
		$headings[] = "<a href=\"admincp/admininfraction.php?do=dolist" . $args . "date\" title=\"" . $vbphrase['order_by_date'] . "\">" . $vbphrase['date'] . "</a>";
		$headings[] = $vbphrase['infraction_type'];
		$headings[] = "<a href=\"admincp/admininfraction.php?do=dolist" . $args . "points\" title=\"" . $vbphrase['order_by_points'] . "\">" . $vbphrase['points_ginfraction'] . "</a>";
		$headings[] = "<a href=\"admincp/admininfraction.php?do=dolist" . $args . "reputation_penalty\" title=\"" . $vbphrase['order_by_reputation_penalty'] . "\">" . $vbphrase['reputation_penalty'] . "</a>";
		$headings[] = "<a href=\"admincp/admininfraction.php?do=dolist" . $args . "expires\" title=\"" . $vbphrase['order_by_expiration'] . "\">" . $vbphrase['expires_ginfraction'] . "</a>";
		...
		foreach ($infractions AS $infraction)
		{
			...
			if($infraction['action'] != 2)
			{
				$url = "admininfraction.php?do=reverse&nodeid=$infraction[nodeid]" . $args . $vbulletin->GPC['orderby'];
				$actions .= construct_link_code($vbphrase['reverse_ginfraction'], $url, false, '', true);
			}
			...
			$url = "admininfraction.php?do=deleteinfraction&nodeid=$infraction[nodeid]" . $args . $vbulletin->gpc['orderby'];
			$actions .= construct_link_code($vbphrase['delete'], $url, false, '', true);
