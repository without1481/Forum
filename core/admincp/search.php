...
define('CVS_REVISION', '$RCSfile$ - $Revision: 92997 $');
...
function admincp_search_get_search_results($assertor, $terms)
{
	...
	$rows = $assertor->assertQuery('admincpSearch', array('terms' => $terms));
	foreach($rows AS $row)
	{
		...
		elseif ($row['fieldname'] == 'cphome' || $row['fieldname'] == 'cpglobal')
		{
			if (isset($navcpMap[$varname]))
			{
				$tmp_results["var_$i"] = array(
					'urls' => array('admincp/' . $navcpMap[$varname]['link']),
...
{
	$info = array (
		'urls' => array('admincp/options.php?do=options&dogroup=' . $group . '#' . $row['optionname']),
