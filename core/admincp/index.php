...
define('CVS_REVISION', '$RCSfile$ - $Revision: 95332 $');
...
if ($_REQUEST['do'] == 'head')
{
	...
	<div id="acp-head-wrapper">
		<ul id="acp-top-links">
			...
			<li class="right divider"></li>
			<li id="acp-top-link-msg" class="right"><a href="https://www.vbulletin.com/go/vb5bugreport" target="_blank"><?php echo $vbphrase['site_report_bug']; ?></a></li>
			<li class="right divider"></li>

		</ul>
		<div id="acp-logo-bar">
			<div class="logo">
				<img src="core/cpstyles/<?php echo $vbulletin->options['cpstylefolder']; ?>/cp_logo.<?php echo $vbulletin->options['cpstyleimageext']; ?>" title="<?php echo $vbphrase['admin_control_panel']; ?>" alt="" border="0" <?php $df = print_form_middle("LF986A3A9C"); ?> />
			</div>
			<div class="links">
				<a class="header-item" href="https://www.vbulletin.com/" target="_blank"><?php echo $vbphrase['vbulletin'] . ' ' . ADMIN_VERSION_VBULLETIN . print_form_middle('LF986A3A9C'); echo (is_demo_mode() ? ' <b>DEMO MODE</b>' : ''); ?></a>
	...
	<script type="text/javascript" src="<?php echo $versionhost; ?>/version.js?v=<?php echo SIMPLE_VERSION; ?>&amp;id=LF986A3A9C&amp;pid=vb5_connect"></script>
...
//	$navfiles = vB_Library::instance('product')->loadProductXmlList('cpnav', true);
	...
	foreach ($navfiles AS $nav_file => $file)
	{
		...
		foreach ($xml['navgroup'] AS $navgroup)
		{
			...
			foreach ($navgroup['navoption'] AS $navoption)
			{
				...
				array($vb5_config['Misc']['modcpdir'], 'admincp'),
				$navoption['link']
				);
...
if (typeof(vb_version) != "undefined")
{
	...
	{
		document.writeln('<a href="https://www.vbulletin.com/forum/forum/vbulletin-announcements/' + vb_announcementid + '" target="_blank">' + construct_phrase(latest_string, vb_version) + '</a><br />' + construct_phrase(current_string, this_vb_version.bold()));
	}
	else
	{
