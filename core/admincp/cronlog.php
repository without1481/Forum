...
define('CVS_REVISION', '$RCSfile$ - $Revision: 92996 $');
...
if ($_REQUEST['do'] == 'view')
{
	...
	if (count($logs))
	{
		$baseUrl = "admincp/cronlog.php?" . vB::getCurrentSession()->get('sessionurl');
		$query = array(
			'do' => 'view',
		...
		if ($vbulletin->GPC['page'] != 1)
		{
			...
			$firstpage = "<input type=\"button\" class=\"button\" tabindex=\"1\" value=\"&laquo; " .
				$vbphrase['first_page'] . "\" onclick=\"vBRedirect('$url');\">";

			$query['page'] = $vbulletin->GPC['page'] - 1;
			$url = htmlspecialchars($baseUrl . http_build_query($query));
		...
		if ($vbulletin->GPC['page'] != $totalpages)
		{
			$query['page'] = $vbulletin->GPC['page'] + 1;
			$url = htmlspecialchars($baseUrl . http_build_query($query));
			$nextpage = "<input type=\"button\" class=\"button\" tabindex=\"1\" value=\"" .
				$vbphrase['next_page'] . " &gt;\" onclick=\"vBRedirect('$url');\">";

			$query['page'] = $totalpages;
			$url = htmlspecialchars($baseUrl . http_build_query($query));
			$lastpage = "<input type=\"button\" class=\"button\" tabindex=\"1\" value=\"" .
				$vbphrase['last_page'] . " &raquo;\" onclick=\"vBRedirect('$url');\">";
		}
