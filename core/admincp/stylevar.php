...
define('CVS_REVISION', '$RCSfile$ - $Revision: 93840 $');
define('NOZIP', true);
	...
	if (count($updated_stylevars))
	{
		...
		foreach ($updated_stylevars AS $stylevarid)
		{
			...
			foreach ($update_properties AS $property)
			{
				...
				{
					// In debug mode, if you set a stylevar part to inherit from, that should
					// always trump a manually set value.
					// Set the stylevar equivalent and remove the prop so the rest of the app will
	...
	echo '<script type="text/javascript">
		...
		(function()
		{
			...
			for (i in stylevarids)
			{
				...
				elm = d.getElementById("varlist_stylevar" + stylevarid);
...
function handle_stylevar_click(e)
{
	...
	if (selected.length != 0)
	{
		...
		var url = "admincp/stylevar.php?" + SESSIONURL + "securitytoken=" + SECURITYTOKEN + "&adminhash=" + ADMINHASH + "&do=fetchstylevareditor&" + request_string + "dostyleid=" + ' . $vbulletin->GPC['dostyleid'] . ';
		var editorpane = YAHOO.util.Dom.get("edit_scroller");
		editorpane.src = url;
	}
}
...
.text-decoration label:after {
	...
	echo '
					...
					<script type="text/javascript" src="core/clientscript/vbulletin_list_filter.js?v=' . SIMPLE_VERSION . '"></script>
					<script type="text/javascript">
