...
define('CVS_REVISION', '$RCSfile$ - $Revision: 95499 $');
...
if ($_REQUEST['do'] == 'domoveattachment')
{
	...
	$filedataQry = vB::getDbAssertor()->assertQuery('filedata', array(
	...
	$resizeQry = vB::getDbAssertor()->assertQuery('filedataresize', array(
			vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_SELECT,
			vB_dB_Query::CONDITIONS_KEY => array(
				array('field' => 'filedataid', 'value' => $vbulletin->GPC['startat'], 'operator' => vB_dB_Query::OPERATOR_GTE),
				array('field' => 'filedataid', 'value' => $finishat, 'operator' => vB_dB_Query::OPERATOR_LT),
	...
	if ($checkmore)
	{
		...
		echo "<p><a href=\"admincp/attachment.php?" . vB::getCurrentSession()->get('sessionurl') .
			"do=domoveattachment&amp;startat=$finishat" .
			"&amp;pp=" . $vbulletin->GPC['perpage'] .
			"&amp;count=" . $vbulletin->GPC['count'] .
			"&amp;attacherrorcount=" . $vbulletin->GPC['attacherrorcount'] . "\">" .
...
if ($_REQUEST['do'] == 'confirmfileremove')
{
	...
	if ($attachments->valid())
	{
		...
		echo "<p><a href=\"admincp/attachment.php?" . vB::getCurrentSession()->get('sessionurl') . "do=confirmfileremove&amp;startat=$finishat&amp;attachtype=" . $vbulletin->GPC['attachtype'] .
											"&amp;pp=" . $vbulletin->GPC['perpage'] . "\">" .
											$vbphrase['click_here_to_continue_processing'] . "</a></p>";
	}
	else
...
if ($_REQUEST['do'] == 'confirmattachmentremove')
{
	...
	if ($vbulletin->GPC['removeattachments'])
	{
		...
		if ($attachments->valid())
		{
			...
			echo "<p><a href=\"admincp/attachment.php?" . vB::getCurrentSession()->get('sessionurl') . "do=confirmattachmentremove&amp;startat=$finishat&amp;removeattachments=1" .
												"&amp;pp=" . $vbulletin->GPC['perpage'] . "\">" .
												$vbphrase['click_here_to_continue_processing'] . "</a></p>";
