...
if ($_REQUEST['do'] == 'method')
{
	...
	if (count($logs) > 0)
	{
		$baseUrl = "admincp/apistats.php?" . vB::getCurrentSession()->get('sessionurl');
		$query = array(
			'do' => 'method',
...
if ($_REQUEST['do'] == 'client')
{
	...
	if (count($logs) > 0)
	{
		...
		foreach ($logs as $log)
		{
			...
			$cell[] = "<input type=\"button\" class=\"button\" value=\"$vbphrase[view_logs]\" onclick=\"vBRedirect('admincp/apilog.php?{$sessionurl_js}do=view&apiclientid=$log[apiclientid]');\" />";
			print_cells_row($cell);
		}
