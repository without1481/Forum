...
vB_Upgrade.prototype.process_step = function(version, step, startat, checktable, response, firstrun, only, htmldata)
{
	...
	//wehre the post doesn't -- which makes it much easier to figure out what's happening
	var url = this.target + "?version=" + PHP.urlencode(version) + "&step=" + step;
...
vB_Upgrade.prototype.process_step_result = function(ajax)
{
	...
	if (ajax.responseXML)
	{
		...
		var scriptXStepYMessage = '<br /><br />' + construct_phrase(SCRIPT_X_STEP_Y, PHP.htmlspecialchars(ajax.argument.version), PHP.htmlspecialchars(ajax.argument.step));
		...
		var skip_no_message = (ajax.responseXML.getElementsByTagName("empty_script").length != 0);
		...
		else if (!prompt.length && !html.length && !skip_no_message)
		{
			this.log_message(construct_phrase(STEP_X_Y, ajax.argument.step, NO_MESSAGE));
		}
...
vB_Upgrade.prototype.process_error_response = function(errors, step, version)
{
	...
	var scriptXStepYMessage = '<br /><br />' + construct_phrase(SCRIPT_X_STEP_Y, PHP.htmlspecialchars(version), PHP.htmlspecialchars(step));

	this.log_message(construct_phrase(STEP_X_Y, PHP.htmlspecialchars(step), FATAL_ERROR_OCCURRED));
...
{
	var safeVersion = PHP.htmlspecialchars(version),
		safeStep = PHP.htmlspecialchars(step),
		safeText = PHP.htmlspecialchars(text),
	...
	if (newtext.length == 0)
	{
		...
		this.log_message(construct_phrase(STEP_X_Y, safeStep, SERVER_NO_RESPONSE));
		this.abort_upgrade();
		return;
	}
	...
	var message = construct_phrase(UNEXPECTED_TEXT, safeText);

	this.log_message(construct_phrase(STEP_X_Y, safeStep, message));
	this.show_confirm(construct_phrase(UNEXPECTED_TEXT, safeText) + scriptXStepYMessage, null, true);
	this.abort_upgrade();
}
