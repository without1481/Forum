	...
	*/
	function step_1()
	{
		if (!$this->field_exists('infractionlevel', 'extend'))
		{
			...
			{
				$adminProPicOpt = 0;
				if(!empty($this->registry->bf_misc_adminoptions['adminprofilepic']))
				{
					$adminProPicOpt = $this->registry->bf_misc_adminoptions['adminprofilepic'];
				}
				$this->run_query(
					sprintf($this->phrase['vbphrase']['update_table'], TABLE_PREFIX . "user"),
					"UPDATE " . TABLE_PREFIX . "user
					 SET adminoptions = adminoptions | " . $adminProPicOpt . "
