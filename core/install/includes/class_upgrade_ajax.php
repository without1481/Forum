...
class vB_Upgrade_Ajax extends vB_Upgrade_Abstract
{
	...
	*/
	...
	{
		parent::init($script);
		$this->custnumber = (strlen('3e3e67d2deb1dc7ecf0185caf55c5a1a') == 32) ? '3e3e67d2deb1dc7ecf0185caf55c5a1a' : md5(strtoupper('3e3e67d2deb1dc7ecf0185caf55c5a1a'));
		...
		if ($this->registry->GPC['ajax'])
		{
			...
			$this->process_step(
				...
				$this->registry->GPC['startat'],
				$this->registry->GPC['checktable'],
				$this->registry->GPC_exists['response'] ? $this->registry->GPC['response'] : null,
				$this->registry->GPC['firstrun'],
				$this->registry->GPC['only'],
				$this->registry->GPC['htmlsubmit'],
				$this->registry->GPC['htmldata'],
				$this->registry->GPC['options']
			);
		}
		else
		{
			...
			if ($proceed)
			{
				if (!empty($_REQUEST['version']) AND $this->registry->GPC['version'] AND ($this->versions[$this->registry->GPC['version']] OR in_array($this->registry->GPC['version'], $this->endscripts, true)))
				{
					$this->scriptinfo = array(
	...
	*/
	protected function begin_upgrade($version, $only = false)
	{
		...
		else
		{
			...
			if (!$this->registry->GPC['confirm'] AND !$this->startup_errors AND $this->startup_warnings)
			{
				$this->htmloptions['startup_warnings'] = '';
				foreach ($this->startup_warnings AS $warning)
				{
					$this->htmloptions['startup_warnings'] .= "<li>$warning</li>";
				}
				$this->htmloptions['login'] = false;
				$this->htmloptions['mismatch'] = false;
				$this->htmloptions['processlog'] = false;
		...
		<head>
			...
			* html .floatcontainer {
			...
			<!--
				...
				var NO_MESSAGE = "<?php echo $this->phrase['upgrade']['no_message']; ?>";
