...
require_once(DIR . "/admincp/global.php");

// #############################################################################

$form_script = 'core/install/tableprefix';
...
if ($_POST['do'] == 'rename')
{
	...
	while ($table = $db->fetch_array($tables, MYSQLI_NUM))
	{
		$valid["$table[0]"] = true;
	}
	unset($table);
...
if ($_POST['do'] == 'confirm')
{
	...
	else
	{
		...
		print_form_header($form_script, 'rename');
...
{
	print_form_header($form_script, 'confirm');
	...
	while($table = $db->fetch_array($tables, vB_Database::DBARRAY_NUM))
	{
...
if ($_REQUEST['do'] == 'modify')
{
	...
	print_form_header($form_script, 'choose');
