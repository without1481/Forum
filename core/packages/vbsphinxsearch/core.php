...
class vBSphinxSearch_Core extends vB_Search_Core
{
	...
	public function getResults(vB_Search_Criteria $criteria)
	{
		...
		{
			// This might have been set by a channel filter. If there was no channel filter,
			// we may need to add it ourselves.
			if (!isset($this->what['is_starter2']))
			{
				$this->what['is_starter2'] = '(id = starter) as is_starter2';
			}
			$orderByIsStarter = 'is_starter2 DESC';
			if (!empty($this->sort))
			{
				$orderByIsStarter .= ", ";
			}
		...
		$query = '
			...
			ORDER BY ' . $orderByIsStarter . implode(', ', $this->sort) . "
				$query_limit
			/**" . __FUNCTION__ . (defined('THIS_SCRIPT') ? '- ' . THIS_SCRIPT : '') . "**/";
...
// 					$this->where[''] = "(" . implode(' OR ', $qbits) . ")";
	...
	protected function process_channel_filters($channelid, $depth = false, $include_starter = false, $depth_exact = false, $excludeUserSpecific = false)
	{
		...
		{
			// This will be used for sorting in getResults()
			if (!isset($this->what['is_starter2']))
			{
				$this->what['is_starter2'] = '(id = starter) as is_starter2';
			}
			/*
			This does not look right. If we're doing a channel filter without include_starter, why would
			we return ONLY starters? However it's not very clear what it's *supposed* to do when
			comparing vs. the DB search analogs. See VBV-17682.
